...
# Pre_CV10: the function was used to obtain the phenotype predictions of training set by 10-fold cross-validation (CV) 
# Pre_test: the function was used to obtain the phenotype predictions of test set

library(rrBLUP)

Pre<-function(phe,A,loc){
phe_M<-phe
phe_M[loc]<-NA
data <-data.frame(y=phe_M,gid=rownames(A))
ans <-kin.blup(data=data,geno="gid",pheno="y",K=A)
pl_te<-ans$g[loc]+mean(phe[-loc])
pcor_te<-cor(as.numeric(phe[loc]),pl_te)
Model=list(pcor_te=pcor_te,pl_te=pl_te) 
return(Model)
}

Pre_CV10<-function(phe,A_train,num_test){
A <-A_train
res1<-Pre(phe,A,1:num_test)
res2<-Pre(phe,A,(num_test+1):(num_test*2))
res3<-Pre(phe,A,(num_test*2+1):(num_test*3))
res4<-Pre(phe,A,(num_test*3+1):(num_test*4))
res5<-Pre(phe,A,(num_test*4+1):(num_test*5))
res6<-Pre(phe,A,(num_test*5+1):(num_test*6))
res7<-Pre(phe,A,(num_test*6+1):(num_test*7))
res8<-Pre(phe,A,(num_test*7+1):(num_test*8))
res9<-Pre(phe,A,(num_test*8+1):(num_test*9))
res10<-Pre(phe,A,(num_test*9+1):nrow(A))
...
Pre_test<-function(trainop,A_test){
A <- A_test
...
data_Met<-apply(data_Met0,2,function(x){
...
train_id<-id[which(id[,2]=="train"),1]
...
prediction_train[,i]<-Pre_CV10(trait_train[,i],K_train,num_CV_test)
}
trait_train<-apply(trait_train,2,as.numeric)
prediction_test<-matrix(0,nrow=num_test,ncol=ncol(trait_all))
K_full<-kinship
for (i in 1:ncol(trait_all)){
prediction_test[,i]<-Pre_test(trait_train[,i],K_full)
}
